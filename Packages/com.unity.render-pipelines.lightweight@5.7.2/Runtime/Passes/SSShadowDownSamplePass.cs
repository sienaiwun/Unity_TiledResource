...
{
    internal class SSShadowDownSamplePass : ScriptableRenderPass
    {
        Material m_SSDownSampleMaterial;
        RenderTargetHandle m_ScreenSpaceShadowmap;
        RenderTextureDescriptor m_RenderTextureDescriptor;
        const string m_ProfilerTag = "Shadow down sample";
        ...
        public void Setup(RenderTextureDescriptor baseDescriptor, RenderingData renderingData)
        {
            ...
            int ratio = renderingData.shadowData.ssShadowDownSampleSize;
            m_RenderTextureDescriptor.width /= ratio;
            m_RenderTextureDescriptor.height /= ratio;
        }
        ...
        {
            if (m_SSDownSampleMaterial == null)
            {
                Debug.LogErrorFormat("Missing {0}. {1} render pass will not execute. Check for missing reference in the renderer resources.", m_SSDownSampleMaterial, GetType().Name);
                return;
            }
            ...
            Blit(cmd, screenSpaceOcclusionTexture, screenSpaceOcclusionTexture, m_SSDownSampleMaterial);
