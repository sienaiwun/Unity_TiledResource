...
#ifdef SS_DOWNSAMPLE
SCREENSPACE_TEXTURE(_ScreenSpaceShadowmapDownSampleTexture);
SAMPLER(sampler_ScreenSpaceShadowmapDownSampleTexture);
#else
...
half SampleScreenSpaceShadowmap(float4 shadowCoord)
{
    ...
    shadowCoord.xy = UnityStereoTransformScreenSpaceTex(shadowCoord.xy);
        ...
        half attenuation = SAMPLE_TEXTURE2D_ARRAY(_ScreenSpaceShadowmapDownSampleTexture, sampler_ScreenSpaceShadowmapDownSampleTexture, shadowCoord.xy, unity_StereoEyeIndex).x;
    #else
        half attenuation = SAMPLE_TEXTURE2D(_ScreenSpaceShadowmapDownSampleTexture, sampler_ScreenSpaceShadowmapDownSampleTexture, shadowCoord.xy).x;
    #endif
#else
    #if defined(UNITY_STEREO_INSTANCING_ENABLED) || defined(UNITY_STEREO_MULTIVIEW_ENABLED)
