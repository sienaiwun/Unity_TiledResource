...
namespace UnityEditor.Rendering.LWRP
{
    ...
    class LightweightRenderPipelineLightEditor : LightEditor
    {
        ...
        class Styles
        {
            ...
            public static readonly GUIContent ShadowDistance = EditorGUIUtility.TrTextContent("阴影范围", "Controls the value for the near shadow distance when rendering shadows. Currently clamped to 0.1 units or 1% of the lights range property, whichever is lower.");
            public static GUIContent supportsShadowLayer = EditorGUIUtility.TrTextContent("设置分层阴影值", "Shadow Layer Surport");
        ...
        private bool m_has_addtional_data;
        ...
        SerializedProperty m_ShadowLayerCullingSurportProp;
        SerializedProperty m_ShadowLayerCullingDistancesProp;
        ...
        void ShadowsGUI()
        {
            ...
            using (var group = new EditorGUILayout.FadeGroupScope(show * m_AnimRuntimeOptions.faded))
            {
                if (group.visible)
                {
                    ...
                    if(m_has_addtional_data)
                    { 
                        m_ShadowDistanceProp.floatValue = Mathf.Max(0.0f, EditorGUILayout.FloatField(Styles.ShadowDistance, m_ShadowDistanceProp.floatValue));
                        EditorGUILayout.PropertyField(m_ShadowLayerCullingSurportProp, Styles.supportsShadowLayer);
                        if (m_ShadowLayerCullingSurportProp.boolValue)
                        {
                            for (int i = 0; i < m_ShadowLayerCullingDistancesProp.arraySize; i++)
                            {
                                SerializedProperty property = m_ShadowLayerCullingDistancesProp.GetArrayElementAtIndex(i);
