...
namespace UnityEditor.Rendering
{
    ...
    public class MaterialUpgrader
    {
        ...
        public static void SetMaterialKeywords(Material material)
        {
            ...
            CoreUtils.SetKeyword(material, "_METALLICSPECGLOSSMAP", false);
            CoreUtils.SetKeyword(material, "_SPECULARHIGHLIGHTS_OFF", false);
            CoreUtils.SetKeyword(material, "_ENVIRONMENTREFLECTIONS_OFF", false);
            CoreUtils.SetKeyword(material, "_OCCLUSIONMAP", false);
            CoreUtils.SetKeyword(material, "_NORMALMAP", false);
        ...
        public static void ClearKeyWords()
        {
            ...
            if (selection == null)
            {
                EditorUtility.DisplayDialog(DialogText.title, "没有选择文件夹", DialogText.ok);
                return;
            }
            ...
            {
                var folder = Selection.objects[i];

                if (folder != null)
                {
                    var path = AssetDatabase.GetAssetPath(folder.GetInstanceID());
            ...
            int selectedFoldersCount = selectedFolders.Count;
            ...
            string[] getGuids = AssetDatabase.FindAssets("t:Material", selectedFolders.ToArray());
            int MaterialNum = 0;
            foreach (string guid in getGuids)
            {
                Material material = AssetDatabase.LoadAssetAtPath(AssetDatabase.GUIDToAssetPath(guid), typeof(Material)) as Material;
                if(material.shader.name.Contains("Batchable"))
                {
                    if (material != null)
                    {
                        MaterialNum++;
            ...
            if (MaterialNum == 0)
            {
                EditorUtility.DisplayDialog(DialogText.title, "该文件夹下没有材质文件", DialogText.ok);
                return;
            }
        }
    }
